    ...
    <header class="bg-white shadow">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center">
                ...
                <router-link :to="{ name: 'AdminUsers' }" class="
                    py-2
                    px-3
                    font-semibold
                    focus:outline-none
...
const allUserLoading = computed(() => store.state.currentAlluser.loading);
...
watch(
    () => store.state.currentAlluser.data,
    (newVal, oldVal) => {
        model.value = {
...
if (route.params.id) {
    store.dispatch("getAlluser", route.params.id);
}
